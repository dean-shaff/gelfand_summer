...
#Physical Constants=====================
h = float(6.62606885e-27) #planks constant
...
def main():
	...
	def photon_index():
		...
		for i in xrange(0,len(dataphot2)):
			...
			def power_law_plot(j):	
				...
				if i == int(j):
					...
					plt.plot(avenerg2to10,np.exp(k_actual)*f(avenerg2to10),'b')
					plt.grid(True)
					plt.show()
			def linear_plot(j):
				if i == int(j):
					fig = plt.figure(figsize=(13,8))
					ax = fig.add_subplot(111)
					y = [float(k_actual-m*logavenerg[t]) for t in xrange(0,len(logavenerg))]
			...
			for j in xrange(0,len(logdNdE)):
				gamma_actual = np.absolute(float((-logdNdE[j]+k_actual)/logavenerg[j]))
		...
		photon_index_graph.create_figure_logxory(time,y_data,y_lim=[2,5],xlog=True,error_ydata=photon_indices,y_error=deviations)
	#photon_index()
	def spectral_index():
		 ...
		 for j in xrange(0,len(dataphot2)):
		 	...
		 	luminositiesradio = [luminosities[i] for i in indices_radio]
		 	logluminosities = [np.log(luminosities[i]) for i in indices_radio]
		 	...
		 	def plot_log(t):
		 		if int(j) == int(t):
			 		...
			 		ax = fig.add_subplot(111)
			 		plt.plot(log_radio,f_2(log_radio),'b')
